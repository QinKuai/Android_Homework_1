...
//	    }
	...
	@RequestMapping("/user/{userid}/header")
	public ResponseEntity<UrlResource> getUserHeader(@PathVariable("userid") Integer userID, @RequestHeader HttpHeaders headers) throws Exception {
		ElearnUser user = userDap.selectByPrimaryKey(userID);
		return getImage(user.getHeaderURL(), headers);
	}
}
